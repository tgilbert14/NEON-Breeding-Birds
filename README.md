...
- **Bird Board** (flagship) — every species as a dot: **ubiquity** (% of points where detected — the least detection-biased axis) × **detection index**. Tap to pin a card; faint dots are too few detections to place.
- **Species Profile** — a downloadable card (PNG + CSV): detection index, ubiquity, points/grids, the **detection-decay** (detections by distance — the detectability signature), yearly counts.
...
## Honesty notes
Detection index ≠ population (labelled everywhere); ubiquity (incidence) is the less-biased axis; Chao2 is incidence-based; lat/long are grid centroids (the map aggregates to grid); the basic package has no family/native-status, so the board colours by detection method. Built by Desert Data Labs · desertdatalabs@gmail.com. Not affiliated with NEON/Battelle/NSF.
